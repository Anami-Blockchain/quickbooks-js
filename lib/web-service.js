...
/**
 * A delegate to handle authentication
 *
 *
 * @type {Object}
 */
var qbAuthentication = new Object();
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function (args, callback) {
    ...
    var auth = false;
    if (typeof qbAuthentication.authenticate === 'function') {

        qbAuthentication.authenticate(args.strUserName.trim(), args.strPassword.trim(), function (err, data) {

            console.log("Error: ", err);
            console.log("in web service authenticate")
            console.log('companyFile ==', data);
            if (data) {
                auth = true;
                console.log("auth true ")
                authenticateData(auth, data);
            } else {
                auth = args.strUserName.trim() === username && args.strPassword.trim() === password;
            }

        });
    }

    function authenticateData(auth, data) {

        var authReturn = [];
        authReturn[0] = data.ticket;
        if (auth) {
            console.log('auth ==', auth);
            // Check if qbXMLHandler responds to method.
            if ((typeof qbXMLHandler.fetchRequests === "function")) {
                qbXMLHandler.fetchRequests(authReturn[0], data, function (err, requests) {
                    requestQueue[authReturn[0]] = requests;
                    if (err || requestQueue[authReturn[0]].length === 0) {
                        authReturn[1] = 'NONE';
                    } else {
                        authReturn[1] = data.companyFile;
                    }
                    console.log('company file ==', data.companyFile);
                    callback({
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.sendRequestXML = function (args, callback) {
    Fiber(function () {
        ...
        while (request === null) {
            ...
            var totalRequests = requestQueue[ticket].length;

            if (counter < totalRequests) {
                var request_generator = requestQueue[ticket][counter];
                try {
            ...
            if (request !== null) {
                console.log('XML to be sent', request);
                callback({
            ...
            console.log("Internal request - no XML generated");
            if ((typeof qbXMLHandler.handleResponse === "function")) {
                qbXMLHandler.handleResponse(ticket, null);
            }
        }
    }).run();
};
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML = function (args, callback) {
    Fiber(function () {
        ...
        if (hresult) {
            ...
            if ((typeof qbXMLHandler.handleResponse === "function")) {
                qbXMLHandler.handleResponse(ticket, response);
            }
            percentage = (!requestQueue[ticket].length) ? 100 : counter * 100 / requestQueue[ticket].length;
            if (percentage >= 100) {
...
module.exports = {
    ...
    setAuthHandler: function (xmlHandler) {
        qbAuthentication = xmlHandler;
    }
};
